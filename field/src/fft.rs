...
/// Decomposes N = N1 × N2, does row DFTs (fit in cache), twiddle, transpose,
/// column DFTs (fit in cache), transpose back. Eliminates cache-miss butterfly
/// stages that dominate large classic FFTs.
fn fft_four_step<F: Field>(values: &mut [F], root_table: &FftRootTable<F>) {
    let n = values.len();
    let lg_n = log2_strict(n);

    // Split: N1 rows × N2 cols, N2 >= N1
    let lg_n1 = lg_n / 2;
    let lg_n2 = lg_n - lg_n1;
    let n1 = 1usize << lg_n1;
    let n2 = 1usize << lg_n2;

    // Build sub-FFT root tables
    let rt2 = fft_root_table::<F>(n2);
    let rt1 = fft_root_table::<F>(n1);

    // Step 1: FFT each row (length N2, contiguous in memory)
    for i in 0..n1 {
        let row = &mut values[i * n2..(i + 1) * n2];
        fft_classic(row, 0, &rt2);
    }

    // Step 2: Twiddle factors — multiply element [i][j] by w_N^(i*j)
    // w_N = primitive N-th root of unity
    // Use root_table[lg_n - 1] which contains w_N^0, w_N^1, ..., w_N^(N/2-1)
    // But we need w_N^(i*j) for i=0..N1, j=0..N2.
    // Compute row-by-row: for row i, multiply col j by w_N^(i*j).
    let g = F::primitive_root_of_unity(lg_n);
    for i in 1..n1 {
        let gi = g.exp_u64(i as u64); // w_N^i
        let mut w = gi; // w_N^(i*1) — skip j=0 (w^0 = 1)
        for j in 1..n2 {
            values[i * n2 + j] *= w;
            w *= gi;
        }
    ...
    // Step 3: Transpose N1 × N2 → N2 × N1 (cache-friendly tiled transpose)
    let mut scratch = vec![F::ZERO; n];
    transpose_rect(values, &mut scratch, n1, n2);
    values.copy_from_slice(&scratch);

    // Step 4: FFT each row of transposed matrix (length N1, contiguous)
    for j in 0..n2 {
        let row = &mut values[j * n1..(j + 1) * n1];
