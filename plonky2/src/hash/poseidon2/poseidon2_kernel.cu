// Poseidon2 CUDA kernel — exact plonky2 constants, Goldilocks field
// Embedded as PTX source, compiled at runtime via NVRTC (no nvcc at build time)
__device__ __constant__ unsigned long long EXT_CONSTS[] = {
...
__device__ __constant__ unsigned long long INT_CONSTS[] = {
...
__device__ __constant__ unsigned long long DIAG[] = {
...
__device__ __forceinline__ unsigned long long gl_reduce128(unsigned __int128 x) {
    unsigned long long lo=(unsigned long long)x, hi=(unsigned long long)(x>>64);
    unsigned long long hi_hi=hi>>32, hi_lo=hi&GL_EPS;
    unsigned long long t0=lo-hi_hi; if(lo<hi_hi) t0-=GL_EPS;
    unsigned __int128 t1=(unsigned __int128)hi_lo*GL_EPS;
    unsigned __int128 t2=(unsigned __int128)t0+t1;
    return (unsigned long long)t2+(unsigned long long)(t2>>64)*GL_EPS;
}
__device__ __forceinline__ unsigned long long gl_add(unsigned long long a, unsigned long long b) {
    unsigned __int128 s=(unsigned __int128)a+b;
    return (unsigned long long)s+(unsigned long long)(s>>64)*GL_EPS;
}
...
__device__ __forceinline__ unsigned long long gl_mac(unsigned long long acc, unsigned long long x, unsigned long long y) {
    return gl_reduce128((unsigned __int128)acc+(unsigned __int128)x*y);
}
__device__ __forceinline__ unsigned long long gl_sbox(unsigned long long x) {
    unsigned long long x2=gl_mul(x,x),x4=gl_mul(x2,x2);
    return gl_mul(gl_mul(x,x2),x4);
}
__device__ void ext_linear(unsigned long long s[WIDTH]) {
    for(int g=0;g<3;g++) {
        int b=g*4;
        unsigned long long t01=gl_add(s[b],s[b+1]),t23=gl_add(s[b+2],s[b+3]);
        unsigned long long t=gl_add(t01,t23),x0=s[b],x2=s[b+2];
...
__device__ void int_linear(unsigned long long s[WIDTH]) {
    unsigned __int128 acc=0;
    for(int i=0;i<WIDTH;i++) acc+=s[i];
    unsigned long long lo=(unsigned long long)acc,hi=(unsigned long long)(acc>>64);
    unsigned long long sum=lo+hi*GL_EPS; if(sum<lo) sum+=GL_EPS;
    for(int i=0;i<WIDTH;i++) s[i]=gl_mac(sum,s[i],DIAG[i]);
}
__device__ void permute(unsigned long long s[WIDTH]) {
    ext_linear(s);
    ...
    for(int r=0;r<ROUNDS_P;r++) { s[0]=gl_add(s[0],INT_CONSTS[r]); s[0]=gl_sbox(s[0]); int_linear(s); }
...
// two_to_one: state=[left[4]||right[4]||0..0], permute, output=state[0..4]
extern "C" __global__ void two_to_one_batch(
    const unsigned long long* __restrict__ in, unsigned long long* __restrict__ out, unsigned int batch) {
    unsigned int idx=blockIdx.x*blockDim.x+threadIdx.x;
    if(idx>=batch) return;
    unsigned long long s[WIDTH];
    for(int i=0;i<8;i++) s[i]=in[idx*8+i];
    for(int i=8;i<WIDTH;i++) s[i]=0;
    permute(s);
    for(int i=0;i<NUM_OUT;i++) out[idx*NUM_OUT+i]=s[i];
}
// hash_no_pad: sponge absorption, RATE=8, output=state[0..4]
extern "C" __global__ void hash_no_pad_batch(
    const unsigned long long* __restrict__ in, unsigned long long* __restrict__ out,
    unsigned int batch, unsigned int leaf_len) {
    unsigned int idx=blockIdx.x*blockDim.x+threadIdx.x;
    if(idx>=batch) return;
    const unsigned long long* leaf=in+idx*leaf_len;
    unsigned long long s[WIDTH]={0};
    unsigned int pos=0;
    while(pos+RATE<=leaf_len) {
        for(int i=0;i<RATE;i++) s[i]=gl_add(s[i],leaf[pos+i]);
        permute(s); pos+=RATE;
    }
    unsigned int rem=leaf_len-pos;
    for(unsigned int i=0;i<rem;i++) s[i]=gl_add(s[i],leaf[pos+i]);
    if(rem>0) permute(s);
    for(int i=0;i<NUM_OUT;i++) out[idx*NUM_OUT+i]=s[i];
}
